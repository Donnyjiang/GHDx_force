...
with open("/home/jiangdingyi/jdy/force/DCF/data/DD3.csv","r") as f:
    data = csv.reader(f)
    #txt = open("/home/jiangdingyi/jdy/force/data/DD6.txt","w")
    for row in data:
        #if b[int(row[0])][int(row[1])] == 0 and b[int(row[1])][int(row[0])] == 0:
            # txt.write('''{source:'''+ row[0] +''',target:'''+ row[1] +''',value:'''+ row[2] +'''},''')
        count[int(row[0])][int(row[1])-206] += 1


with open("/home/jiangdingyi/jdy/force/DCF/data/DD4.csv","r") as f:
    data = csv.reader(f)
    #txt = open("/home/jiangdingyi/jdy/force/data/DD6.txt","w")
    for row in data:
        #if b[int(row[0])][int(row[1])] == 0 and b[int(row[1])][int(row[0])] == 0:
...
with open("/home/jiangdingyi/jdy/force/DCF/data/DD5.csv","r") as f:
    data = csv.reader(f)
...
print(count)
with open("/home/jiangdingyi/jdy/force/DCF/result/result.csv", 'w') as f:
    write = csv.writer(f)
